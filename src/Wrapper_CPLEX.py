...
class Wrapper_CPLEX:
    def __init__(self, symmetry_breaker="None", solver_id="cplex"):
    ...
    def solve(
        ...
        CPLEXSolver = src.smt.getSolver(self.solver_id)
        ...
        if out:
            CPLEXSolver.init_problem(problem, "optimize", sb_option=self.symmetry_breaker,
                                   cplex=f"../Output/CPLEX/" + application_model_json["application"] + "_" + str(uuid.uuid4()),
                                   cplexSol=f"../Output/CPLEX/" + application_model_json["application"] + "_" + str(uuid.uuid4())+".out"
                                     )
        else:
            CPLEXSolver.init_problem(problem, "optimize", sb_option=self.symmetry_breaker)
        print("mode ", mode)
        ...
        if mode == "init" and matrix_init is not None and VMSpecs_init is not None:
            print("init CPLEX from prev ...")
            #add_init_values(CPLEXSolver, matrix_init, VMSpecs_init)
        elif mode == "init":
            print("in CPLEX-init add_init_values")
            #add_init_values(CPLEXSolver)
        if prediction is not None:
            print("in add_pred_soft_constraints")
            #add_pred_soft_constraints(CPLEXSolver, prediction)
        elif prediction_init is not None:
            print("in add_init_values")
            #add_init_values(CPLEXSolver, prediction_init)
        price, distr, runtime, a_mat, vms_type = CPLEXSolver.run()
        ...
        if not runtime or runtime > 10000:
            ...
            output = {
                "output": {
                    ...
                    "time (secs)": runtime,
                    "types_of_VMs": [vm_type for vm_type in vms_type],
                    "prices_of_VMs": distr,
                    "VMs specs": vm_specs,
                    "assign_matr": [[el for el in row] for row in a_mat],
                    "offers": offers_json
                }
            }
